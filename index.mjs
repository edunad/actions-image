...
async function run() {
  ...
  try {
    ...
    if (!files || files.length <= 0)
      return core.setFailed(`Failed to find files on path {${pathGlob}}`);
    ...
    const promises = urls.map(
      (urlData) =>
        new Promise((resolve, reject) => {
          ...
          const base64Decode = Buffer.from(cleanFile, "base64").toString(
            "ascii"
          );
          if (base64Decode.indexOf(annotationTag) === -1)
            return resolve({ imageUrl: urlData.url });

          const fileData = base64Decode.split(annotationTag);
          if (!fileData || fileData.length < 1)
            return reject(
              `Invalid annotation file name, should be {filePath${annotationTag}line:col}`
            );
          ...
          if (!lineCol || lineCol.length !== 2)
            return reject("Invalid annotation file name");
    ...
    const annotationData = await Promise.all(promises).catch((err) =>
      core.setFailed(err)
    );
    if (!annotationData || annotationData.length <= 0)
      return core.setFailed("Failed to generate comments / annotations");
